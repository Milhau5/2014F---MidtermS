...
#pragma region WRITE CONFIGURATION
	String sRoot = m_pSystem->m_pFolder->GetFolderRoot();
	String App = m_pSystem->ApplicationName;
	App = sRoot + App + ".cfg";
...
{
	//Init the Application properties
	m_bForceNewConfig = true;
	m_pSystem->WindowWidth = 720;
	m_pSystem->WindowHeight = 720;
	m_pSystem->WindowName = "Midterm";
	m_pSystem->m_RenderingContext = OPENGL3X;
...
void ApplicationClass::InitAppInternalVariables()
{
	...
	m_pMeshMngr->m_pModelMngr->LoadModelUnthreaded("Planets\\03_Earth.obj", "Earth", glm::rotate(matrix4(), 165.0f, vector3(0.0, 1.0f, 0.0f)));

	m_pMeshMngr->m_pModelMngr->LoadModelUnthreaded("Minecraft\\MC_Creeper.obj", "Creeper");

	m_pCamera->SetPosition(vector3(0.0f, 0.0f, 12.0f));

	m_m4Creeper = glm::translate(0.0f, 3.0f, 0.0f);
}
...
void ApplicationClass::Update (void)
{
	...
	GenerateBoundingSphere(m_m4Creeper,"Creeper");
